...
if __name__ == "__main__":
    ...
    try:
        ...
        keycloak_endpoint = os.getenv(
            "KEYCLOAK_URL", default="https://auth.excellence-cloud.dev"
        )
        ...
        if auth_type == "PUBLIC":
            ...
            if not st.session_state.authenticated:
                ...
                login_background = (
                    f'{os.getenv("PROJECT_ROOT")}/{streamlit_structure.LOGIN_BACKGROUND}'
                    if hasattr(streamlit_structure, "LOGIN_BACKGROUND")
                    else LUND_BG
                )
                login_logo = (
                    f'{os.getenv("PROJECT_ROOT")}/{streamlit_structure.LOGIN_LOGO}'
                    if hasattr(streamlit_structure, "LOGIN_LOGO")
                    else LUND_LOGO
                )
                login_title = (
                    streamlit_structure.LOGIN_TITLE
                    if hasattr(streamlit_structure, "LOGIN_TITLE")
                    else "Welcome to the Dashboard"
                )
                login_text = (
                    streamlit_structure.LOGIN_TEXT
                    if hasattr(streamlit_structure, "LOGIN_TEXT")
                    else "Transforming finance one byte at a time..."
                )
                ...
                with container.container():
                    ...
                    with col2:
                        ...
                        with st.container(border=True):
                            ...
                            keycloak = login(
                                url=os.getenv(
                                    "KEYCLOAK_URL", "https://auth.excellence-cloud.dev"
                                ),
                                realm=os.getenv("KEYCLOAK_REALM", "lex"),
                                client_id=os.getenv(
                                    "STREAMLIT_KEYCLOAK_CLIENT_ID", "LEX_LOCAL_ENV"
                                ),
                                auto_refresh=False,
                ...
                if keycloak.authenticated:
                    user = resolve_user(
                        request=None,
                        id_token=keycloak.user_info,
                        rbac=(auth_type == "PRIVATE"),
                    )
                    if user:
