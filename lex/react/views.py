    ...
    if path == "config.js":
        ...
        replacements = {
            "undefined": {  # Only replace 'undefined' entries
                ...
                "REACT_APP_DOMAIN_BASE": os.getenv(
                    "REACT_APP_DOMAIN_BASE", "localhost"
                ),
                "REACT_APP_PROJECT_DISPLAY_NAME": os.getenv(
                    "PROJECT_DISPLAY_NAME", settings.repo_name
                ),
                "REACT_APP_GRAFANA_DASHBOARD_URL": os.getenv(
                    "REACT_APP_GRAFANA_DASHBOARD_URL", "localhost"
                ),
            }
        }
        ...
        for key, value in replacements["undefined"].items():
            content = content.replace(
                f"window.{key} = undefined", f'window.{key} = "{value}"'
            )
