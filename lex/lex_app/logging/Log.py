...
class Log(CalculatedModelMixin, models.Model):
    ...
    def log(function):
        ...
        def wrap(*args, **kwargs):
            ...
            if hasattr(self, "get_log_filter"):
                if (
                    hasattr(function, "delay")
                    and os.getenv("DEPLOYMENT_ENVIRONMENT")
                    ...
                    return_value = function.apply_async(
                        args=args, kwargs=kwargs, task_id=str(calculation_id)
                    )
    ...
    def calculate(self, *args):
        ...
        val_df_dict_new = {}
        path = (
            "calculation_logs_download/full_logs/"
            + f"""CalculationLogs_{self.group}.xlsx"""
        )
        path_ts = (
            "calculation_logs_download/time_sheets/"
            + f"""TimeSheet_{self.group}.xlsx"""
        )
        path_iv = (
            "calculation_logs_download/input_validation/"
            + f"""InputValidation_{self.group}.xlsx"""
        )
        ...
        for element in self.filter:
            if current_task and os.getenv("CELERY_ACTIVE"):
                ...
                obj = CalculationIDs.objects.filter(
                    calculation_record=f"{args[0]._meta.model_name}_{args[0].pk}"
                ).first()
                calculation_id = getattr(obj, "calculation_id", "test_id")
            logs = pd.DataFrame.from_records(
                CalculationLog.objects.filter(
                    calculationId=calculation_id, method__contains=element
                )
                .values()
                .order_by("timestamp")
            )
            ...
            if len(logs) > 0 and "create" not in calculation_id:
                ...
                if new.shape[1] == 1:
                    message_df = pd.DataFrame(
                        {"Severity": "Success", "Content": new[0]}
                    )
                else:
                ...
                ts = logs.loc[
                    (logs["Severity"] == "Start") | (logs["Severity"] == "Finish")
                ]
                if not ts.empty:
                    # Reset the index
                ...
                input_validation = logs.loc[
                    (logs["message_type"] == "Input Validation")
                ]
        ...
        if not default_storage.exists("calculation_logs_download/time_sheets/"):
            default_storage.save(
                "calculation_logs_download/time_sheets/" + "dummy.txt",
        ...
        if default_storage.exists(path_iv):
            [val_dfs, val_sheet_names] = self.update_dfs_for_excel(
                val_df_dict_new, path_iv
            )

        XLSXField.create_excel_file_from_dfs(
            self.input_validation, path_iv, val_dfs, val_sheet_names
        )
    ...
    def time_tracking(self, logs, level):
        ...
        finish_index = list(
            (same_level_logs[same_level_logs["Severity"] == "Finish"]).index
        )
        # This is a list containing all indices of the start entries
        start_index = list(
            (same_level_logs[same_level_logs["Severity"] == "Start"]).index
        )
        ...
        while i < len(start_index):
            ...
            finish_index_without_time = list(
                logs[
                    (logs["Level"] == level)
                    & (logs["Severity"] == "Finish")
                    & (logs["Time spent [s]"] == 0)
                ].index
            )
