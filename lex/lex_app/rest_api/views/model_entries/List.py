...
class ListModelEntries(ModelEntryProviderMixin, ListAPIView):
    ...
    def filterset_fields(self):
        ...
        try:
            return_fields = {
                f.name: self.get_lookup_expressions(type(f))
                for f in self.kwargs["model_container"].model_class._meta.fields
                if (
                    type(f) in FilterSet.FILTER_DEFAULTS
                    or type(f) in CALCULATION_FIELDS
                )
            }
            ...
            raise APIException(
                {
                    "error": f"Filter fields could not be generated!",
