...
class UserPermission(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in MODIFY_METHODS:
            ...
            if (
                ...
                in inspect.signature(
                    modification_restriction.can_be_modified
                ).parameters
            ):
                if modification_restriction.can_be_modified(
                    obj, user, violations, request.data
                ):
                    return True
            else:
        ...
        if request.method == DELETE_METHOD:
            ...
            if (
                "request_data"
                in inspect.signature(modification_restriction.can_be_deleted).parameters
            ):
                if modification_restriction.can_be_deleted(
                    obj, user, violations, request.data
                ):
                    return True
            else:
                if modification_restriction.can_be_deleted(obj, user, violations):
                    return True
            self.message = get_permission_denied_message(
                "delete", "instance", violations
            )
            return False
