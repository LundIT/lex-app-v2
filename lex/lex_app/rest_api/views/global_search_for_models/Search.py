...
class Search(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for model in self.model_collection.all_containers:
            ...
            if model.id not in EXCLUDED_MODELS and UserPermission().has_permission(
                request=request, view=temp_view
            ):
                fields = model.model_class._meta.get_fields(include_parents=False)
                tempMatch = model.model_class.objects.annotate(
                    search=SearchVector(
                        *[
                            f.name
                            for f in fields
                            if f.get_internal_type() not in EXCLUDED_TYPES
                        ]
                    )
                ).filter(search=query)
                for match in tempMatch:
                    if UserPermission().has_object_permission(
                        request=request, view=temp_view, obj=match
                    ):
                        matchObj = {
                            "id": str(match.pk),
                            "type": model.title,
                            "model": model.id,
                            "url": f"/{model.id}/{match.pk}/show",
                            "content": {
                                "id": str(match.pk),
