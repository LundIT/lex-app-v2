    ...
    def _filter_queryset(self, request, queryset, model_container):
        ...
        if hasattr(model, "modification_restriction"):
            pks = [
                obj.pk
                for obj in queryset
                if model.modification_restriction.can_be_read(
                    instance=obj, user=request.user, violations=[]
                )
            ]
...
def create_filter_queries_from_tree_paths(
    all_filter_queries, filter_node, query_string_so_far
):
    if "entries" in filter_node:
        ...
        for key, value in filter_node["children"].items():
            ...
            create_filter_queries_from_tree_paths(
                all_filter_queries, value, new_query_string
            )
...
class ForeignKeyFilterBackend(filters.BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        if active_filter_tree is not None:
            ...
            create_filter_queries_from_tree_paths(
                all_filter_queries, active_filter_tree, ""
            )
...
class PrimaryKeyListFilterBackend(filters.BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        filter_arguments = (
            {
                model_container.pk_name
                + "__in": list(
                    filter(
                        lambda x: x != "", request.query_params.dict()["pks"].split(",")
                    )
                )
            }
            if "pks" in request.query_params.dict()
            else {}
        )
