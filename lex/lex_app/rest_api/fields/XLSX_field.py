...
class XLSXField(FileField):
    ...
    def split_entries_in_sheet(self, sheet, number_of_inserted_rows, index_len):
        for column_num in range(
            index_len + 1, sheet.max_column + 1
        ):  # Column F starts at index 6 (1-indexed)
            cell = sheet.cell(
                row=number_of_inserted_rows + 1, column=column_num
            )  # Adjust the row number to the row where you want to start splitting
            if cell.value:  # Check if the cell is not empty
    ...
    def create_pivotable_row(
        self,
        sheet,
        index_len,
        number_of_rows_to_be_inserted,
        range_of_pivot_concatenation=None,
    ):
        for column_num in range(index_len + 1, sheet.max_column + 1):
            concatenated_value = ""
            ...
            sheet.cell(
                row=number_of_rows_to_be_inserted + 1,
                column=column_num,
                value=concatenated_value.strip(),
            )

    def create_excel_file_from_dfs(
        self,
        path,
        data_frames,
        sheet_names=None,
        ...
        comments={},
        index=True,
        ranges_of_pivot_concatenation={"default": None},
    ):
        """
        ...
        for df, sheet_name in zip(data_frames, sheet_names):
            if df is not None:
                ...
                df.to_excel(
                    ...
                    freeze_panes=(1, idx_length),
                    index=index,
                )

                worksheet = writer.sheets[sheet_name]  # pull worksheet object
                worksheet_comment = (
                    comments[sheet_name] if sheet_name in comments else None
                )
                cell_formats = {}
                for format in formats:
                    cell_formats[format] = writer.book.add_format(
                        {"num_format": formats[format]}
                    )
                ...
                for idx, col in enumerate(df):  # loop through all columns
                    ...
                    if worksheet_comment is not None:
                        comment = (
                            worksheet_comment[col] if col in worksheet_comment else None
                        )
                    ...
                    worksheet.set_column(
                        idx + idx_length, idx + idx_length, max_len
                    )  # set column width
                    # set Cell format
                    if col in formats:
                        worksheet.set_column(
                        ...
                        worksheet.set_column(
                            ...
                            cell_format=writer.book.add_format(
                                {"num_format": XLSXField.cell_format}
                            ),
                        )
                # Add autofilter:
                if len(df.columns) > 0:
                    worksheet.autofilter(
                        0, 0, len(df), idx_length + len(df.columns) - 1
                    )
