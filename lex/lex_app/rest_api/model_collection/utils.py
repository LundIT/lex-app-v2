...
def get_relation_fields(model: Model):
    ...
    return [
        ...
        if field.get_internal_type() in RELATION_FIELD_TYPES and not field.one_to_many
    ]
...
def enrich_model_structure_with_readable_names_and_types(
    node_name, model_tree, model_collection
):
    ...
    if not model_tree:
        ...
        type = (
            "HTMLReport"
            if HTMLReport in model_class.__bases__
            else "Process"
            if Process in model_class.__bases__
            else "Model"
        )
    ...
    return {
        ...
        "children": {
            sub_node: enrich_model_structure_with_readable_names_and_types(
                sub_node, sub_tree, model_collection
            )
