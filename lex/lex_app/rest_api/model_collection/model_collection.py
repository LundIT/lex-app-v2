...
from lex_app.rest_api.model_collection.utils import (
    ...
    for model_class, process_admin in models2admins.items():
        ...
        if not issubclass(model_class, HTMLReport):
            if model_class._meta.abstract:
                raise ValueError(
                    "The model %s is abstract, but only concrete models can be registered"
...
class ModelCollection:
    ...
    def __init__(self, models2admins, model_structure, model_styling) -> None:
        ...
        self.model_structure = model_structure or {
            "Models": {c.id: None for c in self.all_containers}
        }
