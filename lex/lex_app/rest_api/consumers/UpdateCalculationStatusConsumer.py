...
class UpdateCalculationStatusConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        await self.channel_layer.group_add(
            f"update_calculation_status", self.channel_name
        )
        await self.accept()
    ...
    async def calculation_success(self, event):
        ...
        await self.send(
            text_data=json.dumps({"type": "calculation_success", "payload": payload})
        )

    async def calculation_error(self, event):
        payload = event["payload"]
        await self.send(
            text_data=json.dumps({"type": "calculation_error", "payload": payload})
        )
    ...
    async def calculation_in_progress(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "calculation_in_progress", "payload": payload}
            )
        )
